...
function LaterServer(opts) {
    ...
    prop.internal(this, "tls", opts.tls);
    prop.internal(this, "storage", new RedisStorage({keybase: "later:"}));
...
LaterServer.prototype.replay = function() {
    ...
    function pop() {
        storage.unqueue(function(err, req) {
            if (err) return console.error(String(err.message).red);
            if (!req) return console.info("drained".gray);

            later.deserialize(req).then(function(res) {
                res = String(res.statusCode).magenta + " " + req.method + " " + req.url;
                console.info(res);
            }).catch(function(err) {
                err = String(err.message).red;
                console.error(err);
            });

            // continue to pop all queued messages
            if (req) pop();
        });
    }

    console.info("replaying queued requests".blue);
...
LaterServer.prototype.listen = function(port, hostname, backlog, callback) {
    ...
    // create new server
    server = this.tls
        ? https.createServer(this.tls, this.express)
        : http.createServer(this.express);
    this.servers.push(server);

    // begin listening
    server = server.listen.apply(server, arguments);
    return server;
};
...
LaterServer.prototype.createMiddleware = function() {
    ...
    return function(req, res) {
        if (!later.pathAcceptable(req.path)) {
            console.info("404".yellow + " " + req.path);
            res.writeHead(404, {"Content-Type": "text/plain"});
            res.end("Not Found");
        }

        else if (!later.methodAllowed(req.method)) {
            console.info("405".yellow + " " + req.method);
            res.writeHead(405, {
        ...
        // accept anything else
            ...
            later.store(req)
            .then(function(key) {
                console.info("202".cyan + " " + key);
                res.writeHead(202, {"X-Key": key});
                res.end();
            })
            .catch(function(err) {
                console.error("500".red + " " + err.message);
