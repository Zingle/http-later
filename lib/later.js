        ...
        function unqueue() {
            ...
            later.storage.unqueue(function(err, req, key) {
                if (err) {
                    ...
                    // if request matches sentinal, queue has been exhausted
                    if (sentinel === requtil.hash(req)) return done();

                    // if there's no loop sentinel, set one now
                    sentinel = sentinel || requtil.hash(req);
