...
function sendSerialized(req, done) {
    ...
    // use TLS unless x-later-tls is set to 'insecure'
    scheme = headers.read("X-Later-TLS") === "insecure" ? "http" : "https";
    ...
    retryable(function(opts, done) {
        ...
        stream.end();
    }).retry(6).backoff(function() {   // calibrated to about 30 seconds
